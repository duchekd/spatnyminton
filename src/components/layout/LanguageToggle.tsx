import { IconButton, Tooltip } from "@mui/material";

import useStore from "../../hooks/useStore";

import useTexts from "../../languages";

// Kruhové vlajky (zdroj: circle-flags). Ořez kruhem přes mask s unikátním id.
const CzechFlag = () => (
  <svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" aria-hidden="true">
    <mask id="flagToggleCz">
      <circle cx="256" cy="256" r="256" fill="#fff" />
    </mask>
    <g mask="url(#flagToggleCz)">
      <path fill="#eee" d="M0 0h512v256l-265 45.2z" />
      <path fill="#d80027" d="M210 256h302v256H0z" />
      <path fill="#0052b4" d="M0 0v512l256-256z" />
    </g>
  </svg>
);

const UsFlag = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 512 512" aria-hidden="true">
    <mask id="flagToggleUs">
      <circle cx="256" cy="256" r="256" fill="#fff" />
    </mask>
    <g mask="url(#flagToggleUs)">
      <path
        fill="#eee"
        d="M256 0h256v64l-32 32l32 32v64l-32 32l32 32v64l-32 32l32 32v64l-256 32L0 448v-64l32-32l-32-32v-64z"
      />
      <path fill="#d80027" d="M224 64h288v64H224Zm0 128h288v64H256ZM0 320h512v64H0Zm0 128h512v64H0Z" />
      <path fill="#0052b4" d="M0 0h256v256H0Z" />
      <path
        fill="#eee"
        d="m187 243l57-41h-70l57 41l-22-67zm-81 0l57-41H93l57 41l-22-67zm-81 0l57-41H12l57 41l-22-67zm162-81l57-41h-70l57 41l-22-67zm-81 0l57-41H93l57 41l-22-67zm-81 0l57-41H12l57 41l-22-67Zm162-82l57-41h-70l57 41l-22-67Zm-81 0l57-41H93l57 41l-22-67zm-81 0l57-41H12l57 41l-22-67Z"
      />
    </g>
  </svg>
);

const LanguageToggle = () => {
  const texts = useTexts();
  const { culture, setCulture } = useStore();

  const isCzech = culture === "cs-CZ";
  // tooltip popisuje jazyk, na který se přepne
  const label = isCzech ? texts.english : texts.czech;

  return (
    <Tooltip title={label}>
      <IconButton onClick={() => setCulture(isCzech ? "en-US" : "cs-CZ")} aria-label={label}>
        {isCzech ? <CzechFlag /> : <UsFlag />}
      </IconButton>
    </Tooltip>
  );
};

export default LanguageToggle;
